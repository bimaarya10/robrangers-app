...
export default function AiMentorTab() {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      let aiText = "Pertanyaan bagus! Untuk melakukan itu di Roblox Studio, pastikan kamu mendefinisikan Event trigger dengan benar. Coba jalankan skrip di workspace untuk melihat output log.";
      let newHint = "Ingenious Tip: Connection Events di Roblox bersifat Case-Sensitive.";
      
      if (query.toLowerCase().includes("obby")) {
        aiText = "Untuk Obby dinamis, kamu bisa memanfaatkan Vector3.new() untuk memindahkan posisi part secara otomatis menggunakan Loop. Letakkan part di server script.";
        newHint = "Gunakan while true do wait(1) end untuk loop pergerakan platform.";
      } else if (query.toLowerCase().includes("remoteevent")) {
        aiText = "RemoteEvent digunakan untuk menjembatani script Client (LocalScript) dan script Server. Gunakan :FireServer() dari Client, dan :Connect() di Server.";
        newHint = "Jangan meletakkan RemoteEvent langsung di Workspace, letakkan di ReplicatedStorage.";
      }

      setMessages(prev => [...prev, { id: `ai-${Date.now()}`, sender: "ai", text: aiText }]);
      setRecommendations(prev => [newHint, ...prev.slice(0, 2)]);
    }, 1200);
  };
